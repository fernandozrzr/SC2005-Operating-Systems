...
int VpnToPhyPage(int vpn)
{
  ...
  //you can refer to PageOutPageIn(int vpn) to see how an entry was created in ipt
   ...
   for (int i = 0; i < NumPhysPages; i++) {
        ...
        // return physical page for a VPN if it exists in the IPT
...
int lruAlgorithm(void)
{
  ...
  int last = memoryTable[0].lastUsed;
    ...
    for (int i = 0; i < NumPhysPages; i++) {
        // check if there is an invalid frame, if so make it the victim page
        if (!memoryTable[i].valid) {
            phyPage = i;
            break;
        }
        // if all pages are valid, use LRU and compare lastUsed time to find victim page
